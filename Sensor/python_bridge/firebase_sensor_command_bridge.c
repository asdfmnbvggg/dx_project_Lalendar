...
// 만약 0x27 계열이 안 되면 아래처럼 바꿔야 할 수도 있음
// 기본: 0x3F
// A0:   0x3E
// A1:   0x3D
...
void printSensorData() {
  ...
  if (scale.is_ready()) {
    ...
    // Python에서 weight가 없으면 Firebase 업로드가 안 될 수 있으므로
    // 기존 구조 유지하려면 0.0이라도 출력
    Serial.print("Weight: ");
    Serial.println(weight, 1);
...
void handleCommand(String command) {
  ...
  // Python에서 예전 LCD 메시지가 들어와도 무시
  // 지금은 LCD 3개에 센서별로 직접 표시하기 때문
  if (command.startsWith("LCD:")) {
    Serial.println("LCD command ignored - using 3 LCD sensor mode");
    return;
  }
...
void setup() {
  ...
  displayLCD(lcdTempHumid, "TEMP / HUMID", "Starting...");
  displayLCD(lcdDust, "DUST SENSOR", "Starting...");
  displayLCD(lcdWeight, "WEIGHT SENSOR", "Starting...");

  Serial.println("ESP32 sensor + command bridge ready");
  Serial.println("3 LCD mode ready");
}
